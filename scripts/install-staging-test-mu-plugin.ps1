...
#   powershell -ExecutionPolicy Bypass -File .\scripts\install-staging-test-mu-plugin.ps1
...
& $plink.Source -batch -pw $password $hostSpec "mkdir -p '$appRoot/wp-content/mu-plugins' '$appRoot/wp-content/mu-plugins/bsp-staging-test-cpt'"
...
Write-Host 'Then seed fixtures (if needed):'
